...
filled_form <- set_values(pgform, email = "XXXX", password ="XXXX")
...
# EJM <- EJM[!grepl(paste(c("Industrial Organization",
#                           "Real Estate"
#                           ), collapse="|"),EJM$Field, ignore.case=TRUE, perl=TRUE, useBytes=TRUE),]
EJM <- EJM %>% filter(Field != ???Real Estate???)
